...
//Code Here
let evenNumbers = mixedNumbers.filter( (element, index, array) => {return element % 2 === 0})
...
//Code Here
let postTaxPrices = prices.map((el, i, arr) => {return el * 1.07});
...
//Code Here
let totalPopulation = populations.reduce((accum, el, i, arr) => accum + el)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((el, i, arr) => {return el.CP > 200})
...
let orderTotals = orders.map((el, i, arr) => {return el.price * (1 + el.tax)})
...
let bobsTotal = bobsPurchaces.reduce((acc, el) => acc + el.price)
